//nolint:testpackage // in-package tests share the validator's package-level constants.
package modem
...
func TestValidateASCII_Accept(t *testing.T) {
	tests := []struct {
		...
		{name: "full printable range", text: printableRangeText()},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := ValidateASCII(tt.text); err != nil {
...
func TestValidateASCII_TooLongError(t *testing.T) {
	err := ValidateASCII(strings.Repeat("x", 161))
	if err == nil {
		t.Fatal("ValidateASCII = nil, want error")
	}
...
// printableRangeText returns every printable 7-bit ASCII rune 0x20-0x7E.
func printableRangeText() string {
	var b strings.Builder
	for r := rune(asciiPrintableMin); r <= asciiPrintableMax; r++ {
		b.WriteRune(r)
	}
