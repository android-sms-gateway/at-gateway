...
docker run -d --name at-gateway \
  --device /dev/ttyUSB0 \
  -v /path/to/at-gateway-data:/app/data \
...
`/app/data` must be persisted (it holds the registered device ID in `data/storage.json`) and writable by UID 1000 (`appuser`), e.g. `chown 1000:1000 /path/to/at-gateway-data`.
...
[license-url]: https://github.com/android-sms-gateway/at-gateway/blob/master/LICENSE
